...
const createPopUp = (html, senctence, x, y, screenX, screenY) => {
  ...
  if (screenX + frame_width < screenWidth) {
    frame_left = x;
  } else {
    frame_left = (x - frame_width - 2 * padding);
  }
  frame.style.left = frame_left + 'px';
  if (screenY + frame_height + 20 < screenHeight) {
    frame_top = y;
  } else {
    frame_top = (y - frame_height - 2 * padding);
  }
  frame.style.top = frame_top + 10 + 'px';
  if (frame.style.left + frame_width > screenWidth) {
    frame.style.left -= frame.style.left + frame_width - screenWidth;
  }
